...
function showPagination(doc, loc, name, curPage, numOfPage)
{
    ...
    if(curPage > 1){
        ...
        back.onclick = function() {
            showSearchResult(doc, name, curPage-1);
        }
    }else{
    ...
    for (let i = 1; i < curPage; i++) {
        ...
        boxKiri.onclick = function() {
            showSearchResult(doc, loc, name, i);
        }
    ...
    if(curPage < numOfPage){
        ...
        next.onclick = function() {
            showSearchResult(doc, loc, name, curPage+1);
        }
    }else{
...
function showSearchResult(doc, loc, name, page) {
    var xmlhttp = new XMLHttpRequest();
    var hasil = "";
    xmlhttp.open("GET", "../php/searchFilm.php?query="+encodeURI(name)+"&page="+page);
    xmlhttp.onload = function () {
        hasil = JSON.parse(xmlhttp.responseText);
        var banyakFilm = hasil["banyakFilm"];
        var numOfPage = hasil["numOfPage"];
        doc.getElementsByClassName("search-number")[0].innerHTML = banyakFilm;
        // console.log(hasil);
        if(numOfPage > 1){
            showPagination(doc, loc, name, page, numOfPage);
        }
        // <div class="film-card">
        //     img container
        //     detail container
